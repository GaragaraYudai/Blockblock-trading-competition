...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetchWithAuth(`${API_URL}/leaderboard`);
      if (!res.ok) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-24 flex items-center justify-between relative">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-400 hidden md:block">
              환영합니다, <span className="text-white font-semibold">{currentUsername}</span>
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            {userIsAdmin && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-semibold rounded-xl transition-all active:scale-95 shadow-lg shadow-purple-500/20"
                title="Admin Panel"
              >
                <Shield size={18} />
            ...
            <button
              onClick={logout}
              className="p-3 bg-red-500/10 hover:bg-red-500/20 rounded-xl border border-red-500/30 transition-all active:scale-95 group shadow-lg shadow-black/20"
              title="Logout"
            >
              <LogOut size={22} className="text-red-400 group-hover:text-red-300 transition-all" />
            </button>
          </div>
        </div>
      </nav>
